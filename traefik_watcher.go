...
func newConsulWatcher(cr *traefikRegistry) (registry.Watcher, error) {
	cw := &traefikWatcher{
		r:        cr,
		exit:     make(chan bool),
...
func (cw *traefikWatcher) serviceHandler(idx uint64, data interface{}) {
...
func (cw *traefikWatcher) handle(idx uint64, data interface{}) {
...
func (cw *traefikWatcher) Next() (*registry.Result, error) {
	select {
	case <-cw.exit:
